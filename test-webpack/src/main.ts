...
function render() {
  ...
  const sections: { label: string; code: string; data: unknown }[] = [
    { label: "User",           code: "createMock<User>()",          data: user  },
    { label: "Post",           code: "createMock<Post>()",          data: post  },
    { label: "PaginatedUsers", code: "createMock<PaginatedUsers>()", data: page  },
    { label: "User × 3",      code: "createMockList<User>(3)",      data: users },
  ]
